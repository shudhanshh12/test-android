...
class GetOnboardingDetails @Inject constructor(private val iplRepository: Lazy<IplRepository>) {

    fun execute(matchId: String): Observable<Result<OnboardingDetails>> {
        return UseCase.wrapSingle(
            rxSingle { iplRepository.get().getOnBoardingDetails(matchId) }
        )
    }
}
