...
class GetBoosterQuestion @Inject constructor(private val iplRepository: Lazy<IplRepository>) {

    fun execute(matchId: String): Observable<Result<BoosterQuestion>> {
        return UseCase.wrapSingle(
            rxSingle { iplRepository.get().getBoosterQuestion(matchId) }
        )
    }
}
