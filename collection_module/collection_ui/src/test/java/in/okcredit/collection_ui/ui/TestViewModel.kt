package `in`.okcredit.collection_ui.ui
...
abstract class TestViewModel<S : UiState, P : UiState.Partial<S>, E : BaseViewEvent> {
    ...
    /**
     * Get all the view events
     */
    fun viewEventValues(): List<E> = viewEffectObserver.values()
     ...
     */
    protected fun pushIntent(
        intent: UserIntent,
        delay: Long = DEFAULT_DELAY_FOR_INTENT,
        awaitCount: Int = DEFAULT_AWAIT_COUNT,
    ) {
    ...
    companion object {
        ...
        private const val DEFAULT_AWAIT_COUNT = 1
    }
}
