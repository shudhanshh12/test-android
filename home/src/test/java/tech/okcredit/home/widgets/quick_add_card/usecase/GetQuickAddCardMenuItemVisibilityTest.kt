...
class GetQuickAddCardMenuItemVisibilityTest {
    ...
    private val getQuickAddCardMenuItemVisibility =
        GetQuickAddCardMenuItemVisibility({ ab }, { quickAddCardRepository }, { getActiveBusinessId })
    ...
    @Test
    fun `Usecase Should Return false when ab is enabled but quickAddCard Shares Preference is false`() {
        // Given
        whenever(getActiveBusinessId.execute()).thenReturn(Single.just(businessId))
        whenever(ab.isFeatureEnabled(QuickAddCardHelper.FEATURE_KEY, businessId = businessId)).thenReturn(Observable.just(true))
        whenever(quickAddCardRepository.shouldShowQuickAddCard(businessId)).thenReturn(Observable.just(true))

        // When
        val testObserver =
            getQuickAddCardMenuItemVisibility.execute().subscribeOn(Schedulers.trampoline()).test()
    ...
    fun `Usecase Should Return false when ab is not enabled and quickAddCard Shares Preference is false`() {
        // Given
        whenever(getActiveBusinessId.execute()).thenReturn(Single.just(businessId))
        whenever(ab.isFeatureEnabled(QuickAddCardHelper.FEATURE_KEY, businessId = businessId)).thenReturn(Observable.just(false))
    ...
    @Test
    fun `Usecase Should Return false when ab is enabled`() {
        // Given
        whenever(getActiveBusinessId.execute()).thenReturn(Single.just(businessId))
        whenever(ab.isFeatureEnabled(QuickAddCardHelper.FEATURE_KEY, businessId = businessId)).thenReturn(Observable.just(true))
        whenever(quickAddCardRepository.shouldShowQuickAddCard(businessId)).thenReturn(Observable.just(false))
