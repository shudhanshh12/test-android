...
class AuthenticateNewOtp @Inject constructor(private val authService: AuthService) : UseCase<Credential, Boolean> {
    override fun execute(req: Credential): Observable<Result<Boolean>> {
        return UseCase.wrapSingle(
            Single
                .fromCallable {
                    try {
                        authService.authenticateNewPhoneNumberCredential(req)
